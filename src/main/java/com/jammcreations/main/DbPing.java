...
public class DbPing {
	public static void main(final String[] args) {
		...
		final String home = getEnv("HOME");

		final File lockFile = new File(home + "/DB_PING.lock");
		if (lockFile.exists()) {
			System.out.println("Lockfile found.  Checking if server is back up");
			try {
				pingDb(jdbcUrl);
			} catch (SQLException e) {
				System.out.println("Server still down: " + e.getMessage());
				return;
			}

			try {
				send(gmailAddr, gmailPassword, to, "DB PING SUCCESS", "Looks like the db connection is back up");
		...
		try {
			...
			try {
				send(gmailAddr, gmailPassword, to, "DB PING FAILED", e.getMessage()
						+ "\n\nThis is probably due to the connecting IP having changed.  Do a nslookup jamm.duckdns.org and check against firewall rules.  =)");
	...
	private static void send(final String from, final String password, final String to, final String sub,
			final String msg) throws MessagingException {
